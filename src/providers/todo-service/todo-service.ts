...
export class TodoServiceProvider {
  ...
  public deleteListTodo(listUuid: String) {
  ...
  public addList(name:string){
    this.data.push({uuid:name+Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15),name : name,items:[]});
  }

  public addItem(uuidlist : string, newTodo : TodoItem){
    let items = this.data.find(d => d.uuid == uuidlist).items;
    items.push(newTodo);

  }

  public editList(uuidlist : string, name : string){
    this.data.find(d=> d.uuid == uuidlist).name = name;
  }
}
